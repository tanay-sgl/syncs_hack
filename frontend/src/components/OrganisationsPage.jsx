...
export default function OrganisationsPage(){
  const [organisations,setOrganisations]=useState(mockOrganisations)
  ...
  const allNeeds=organisations.flatMap((organisation)=>[...organisation.openIntents,...customNeeds.filter((need)=>need.organisationId===organisation.id)].map((need)=>({...need,organisation})))
  const filtered=useMemo(()=>{const search=filters.search.toLowerCase();return organisations.filter((org)=>filters.category==='All'||org.category===filters.category).filter((org)=>filters.type==='All types'||[...org.openIntents,...customNeeds.filter((need)=>need.organisationId===org.id)].some((need)=>need.type===filters.type)).filter((org)=>!search||[org.name,org.description,...org.tags,...org.openIntents.flatMap((need)=>[need.title,...need.skillsNeeded])].join(' ').toLowerCase().includes(search))},[filters,customNeeds,organisations])
  return <div className="organisations-page container"><header className="org-header"><div><p className="eyebrow"><span /> ORGANISATION NETWORK</p><h1>Communities looking <span>for people</span></h1><p>Discover organisations, projects and opportunities built around live needs.</p><small>People aren’t the only blocks that need connecting.</small></div><button className="button" type="button" onClick={()=>setCreating(true)}>+ Post a need</button></header>
    ...
    {creating&&<CreateOrganisationNeed organisation={organisations[0]} onClose={()=>setCreating(false)} onSubmit={(need)=>{setCustomNeeds((current)=>[need,...current]);setCreating(false)}} />}
  </div>
}
