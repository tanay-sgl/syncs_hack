...
import { detectClusters, fetchLiveIntents } from '../api/client.js'
...
export default function DiscoverPage() {
  const [liveIntents, setLiveIntents] = useState(mockLiveIntents)
  const fallbackSessions = useMemo(() => detectEmergentSessionsMock(liveIntents), [liveIntents])
  ...
  useEffect(() => {
    let active = true
    fetchLiveIntents()
      .then((intents) => { if (active) setLiveIntents(intents) })
      .catch(() => {})
    return () => { active = false }
  }, [])
  useEffect(() => {
    let active = true
    detectClusters(adaptLiveIntentsToApi(liveIntents))
      .then((response) => {
  ...
  const filtered = useMemo(() => {
    ...
    const result = liveIntents.filter((intent) => !intent.clusterKey)
  ...
  const totalPeople = liveIntents.reduce((sum,intent)=>sum+intent.participantCount,0)

  return <div className="discover-page container"><header className="discover-header"><p className="eyebrow"><span /> LIVE INTENT NETWORK</p><h1>What’s trying to happen <span>around you?</span></h1><p>Explore people, sessions and opportunities forming around shared intent.</p><div className="discover-stats"><article><strong>{totalPeople}</strong><span>live signals</span></article><article><strong>{sessions.length + 11}</strong><span>sessions forming</span></article><article><strong>{liveIntents.filter((item)=>['Projects','Hackathons'].includes(item.category)).length}</strong><span>projects looking for people</span></article><article><strong>{liveIntents.filter((item)=>item.category==='Organisations').length}</strong><span>organisations recruiting</span></article></div></header>
